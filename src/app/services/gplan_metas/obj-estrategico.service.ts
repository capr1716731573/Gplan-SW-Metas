...
export class ObjEstrategicoService {
  url:string=dominio_ws+'/objetivo_estrategico';
  tabla:string='Objetivo Estrategico GADs';

  constructor(public http:HttpClient,
              public _settingsService:SettingsService) { }

  cargarDatos(pk_gad:number){
    let url_ws=`${this.url}/${pk_gad}`;
  ...
  cargarDatosAll(pk_gad:number){
    let url_ws=`${this.url}/todos/${pk_gad}`;
    return this.http.get(url_ws)
    .pipe(map((resp:any) =>{
        let dato={};
        if(resp.status === 'error'){
