...
export class MetasService {
  url:string=dominio_ws+'/meta';
  tabla:string='Metas';

  constructor(public http:HttpClient,
              public _settingsService:SettingsService) { }

  cargarDatos(pk_plan:number){
    let url_ws=`${this.url}/${pk_plan}`;
  ...
  crearMeta(json:any):Observable<any>{
    let url_ws=`${this.url}/crear_metas`;
  ...
  cargarLineaBase(pk_plan:number):Observable<any>{
    let url_ws=`${this.url}/linea_base/${pk_plan}`;
    return this.http.get(url_ws)
    .pipe(map((resp:any) =>{
        let dato={};
        if(resp.status === 'error'){
