...
export class PntvOdsService {
  url:string=dominio_ws+'/pntv';
  tabla:string='Objetivo PNTV ';

  constructor(public http:HttpClient,
              public _settingsService:SettingsService) { }

  cargarDatos(pk_eje:number,pk_ods:number){
    let url_ws=`${this.url}/${pk_eje}/${pk_ods}`;
  ...
  cargarDatosConfig(pk_eje:number,pk_ods:number){
    let url_ws=`${this.url}/config/${pk_eje}/${pk_ods}`;
    return this.http.get(url_ws)
    .pipe(map((resp:any) =>{
        let dato={};
        if(resp.status === 'error'){
